...
class Park extends Element {
    ...
    treeDensity() {
        ...
        console.log(`Tree density of ${this.name} is: ${density} square kilometers.`);
    }
}
...
const allParks = [
    new Park('Shevchenko Park', 1990, 1500, 10),
    new Park('Pushkina Park', 1950, 500, 2),
    new Park('Goloseevo', 1970, 5000, 30)
];
...
function reportParks(parks) {
    ...
    const [sum, averAge] = calculateSumAndAverage(ages);
    console.log(`Our ${parks.length} park(s) has an average age of ${averAge} years.`);
...
function reportStreets(streets) {
    ...
    // Classify streets
    streets.forEach(cur => cur.classifyStreets());

    const [total, average] = calculateSumAndAverage(streets.map(el => el.length));
